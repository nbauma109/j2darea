...
| Bricks and floor tiles | Implemented | Two separate parallelogram choices and editors: `FLOOR TILES` produces flat AR3401-style square stone slabs, while `BRICKS` produces TU0018/BD0117-style small rectangular masonry in running, quarter or stack bond for either floors or walls. Both offer fifteen plainly named natural colours and remember their own settings. Floors carry `STONE` terrain; brick walls behave as ordinary objects and carry none. See [Bricks and Floor Tiles](Brick-Floor-Generator.md). |
| Wallpaper generation | Implemented | `WALLPAPER` produces a canvas-anchored all-over wall repeat with sixteen selectable pattern families, a five-by-three grid of muted three-ink colour schemes, adjustable repeat size and line weight, plus fading, wear and wall lighting. Wallpaper is an ordinary wall object and contributes no terrain. See [Wallpaper Generator](Wallpaper-Generator.md). |
| Window generation | Implemented | `WINDOWS` fits a deeply beveled frame, dark mottled glazing and fine diamond leading to the drawn parallelogram, with one to four panes across and one to three panes down. A visual grid offers fifteen muted frame/glass/fabric schemes; optional curtains use a scalloped valance, tied and flared side drops, aged braid, adjustable opening and length. Screen orientation is normalized so fabric always hangs from the upper edge regardless of vertex order. Windows are ordinary wall objects and contribute no terrain. See [Window Generator](Window-Generator.md). |
| Bookcase generation | Implemented | `BOOKCASE` fits a flat bookcase into the drawn parallelogram: two to seven shelf levels, one to four bays, adjustable frame width and book density, and a five-by-three grid restricted to brown timber. Books use seeded variable-width geometry rather than slots, forming touching clusters, row-specific density, irregular gaps, nearly full-height spines and occasional lean or bands. Orientation is normalized so books always stand on their shelves. Bookcases are ordinary wall objects and contribute no terrain. See [Bookcase Generator](Bookcase-Generator.md). |
| Carpet generation | Implemented | The parallelogram fill selector can weave a randomized geometric carpet, symmetric about both its axes: five field patterns, four border motifs, four medallion sizes and six dye sets, any of which can be pinned or left to the seed, plus fringe, knot-stepped motifs, pile and wear. Laid out in the shape's own frame, so it moves with the object, and pasted over the floors rather than under them. See [Carpet Generator](Carpet-Generator.md). |
